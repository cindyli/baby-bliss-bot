...
def example_use_case_1(engine):
    ...
    print("Example 1a: Look up symbol ID 392 (building)")
    print("-" * 70)
    result = engine.get_symbol_glosses(392, language="en")
    print(json.dumps(result, indent=2, ensure_ascii=False))

    # Example 1b: Look up non-existent symbol
    print("\n\nExample 1b: Look up non-existent symbol (error case)")
    print("-" * 70)
...
def example_use_case_2(engine):
    ...
    print("\n\nExample 2b: Look up new composition [368, 392, 958, 99] ('many hospitals')")
    print("Composition structure:")
    print("  - 368: QUANTIFIER modifier (many)")
    print("  - 392: Classifier (building)")
    print("  - 958: Specifier (medicine)")
    print("  - 99: Indicator (plural)")
    print("-" * 70)
...
def example_utility_methods(engine):
    ...
    print("-" * 70)
    result = engine.get_symbol_info("392")
    print(json.dumps(result, indent=2, ensure_ascii=False))

    # Classify symbols in a composition
    print("\n\nExample 2: Classify symbols in a composition")
    print("-" * 70)
    result = engine.classify_symbols(["368", "392", "958", "99"])
    print(json.dumps(result, indent=2, ensure_ascii=False))
...
def main():
    ...
    dict_paths = [
        "src/data/bliss_dict/bliss_symbol_explanations_english.json",
        "data/bliss_dict/bliss_symbol_explanations_english.json",
        "../data/bliss_dict/bliss_symbol_explanations_english.json",
    ]
    ...
    for path in dict_paths:
        try:
            bliss_dict = load_bliss_dict(path)
    ...
    if bliss_dict is None:
        ...
        print("  bliss_symbol_explanations_english.json exists in the src/data/bliss_dict directory.")
