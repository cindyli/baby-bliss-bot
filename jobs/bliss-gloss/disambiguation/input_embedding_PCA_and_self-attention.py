...
from utils_input_embedding import (
    ...
    get_average_output_embeddings,
    get_unambiguous_embedding_by_PCA_threshold,
    get_unambiguous_embedding_by_PCA_kneed,
    get_unambiguous_embedding_by_PC1_and_variance_contributions,
...
USE_PC1_ON_INPUT_EMBEDDING = True  # If True, use primary principal component on input embedding
...
USE_PC1_ON_OUTPUT_EMBEDDING = True  # If True, use primary principal component on output embedding
USE_SELF_ATTENTION_ON_OUTPUT_EMBEDDING = False  # If True, use self-attention on output embedding
USE_CALCULATED_OUTPUT_EMBEDDING = False  # If True, use average output embedding
...
output_embedding_average = get_average_output_embeddings(model, tokenizer, target_tokens)
...
if IN_COMPARISON_MODE or USE_PCA_THRESHOLD_ON_INPUT_EMBEDDING:
    ...
    input_embedding_PCA_threshold = get_unambiguous_embedding_by_PCA_threshold(input_embeddings_of_target_tokens, explained_variance_threshold)

if IN_COMPARISON_MODE or USE_PCA_KNEED_ON_INPUT_EMBEDDING:
    input_embeddings_of_target_tokens = get_embeddings_of_words(model, tokenizer, target_tokens)
    input_embedding_PCA_kneed = get_unambiguous_embedding_by_PCA_kneed(input_embeddings_of_target_tokens, kneed_sensitivity)

if IN_COMPARISON_MODE or USE_PC1_ON_INPUT_EMBEDDING:
    input_embeddings_of_target_tokens = get_embeddings_of_words(model, tokenizer, target_tokens)
    input_embedding_PC1, variance_contribs = get_unambiguous_embedding_by_PC1_and_variance_contributions(input_embeddings_of_target_tokens)
...
if IN_COMPARISON_MODE or USE_SELF_ATTENTION_ON_INPUT_EMBEDDING:
    input_embeddings_of_target_tokens = get_embeddings_of_words(model, tokenizer, target_tokens)
    input_embedding_self_attention = get_unambiguous_embedding_by_self_attention(input_embeddings_of_target_tokens)
...
if IN_COMPARISON_MODE or USE_PCA_THRESHOLD_ON_OUTPUT_EMBEDDING:
    ...
    output_embedding_PCA_threshold = get_unambiguous_embedding_by_PCA_threshold(output_embeddings_of_target_tokens, explained_variance_threshold)

if IN_COMPARISON_MODE or USE_PCA_KNEED_ON_OUTPUT_EMBEDDING:
    output_embeddings_of_target_tokens = get_embeddings_of_words(model, tokenizer, target_tokens, type="output_embeddings")
    output_embedding_PCA_kneed = get_unambiguous_embedding_by_PCA_kneed(output_embeddings_of_target_tokens, kneed_sensitivity)

if IN_COMPARISON_MODE or USE_PC1_ON_OUTPUT_EMBEDDING:
    output_embeddings_of_target_tokens = get_embeddings_of_words(model, tokenizer, target_tokens, type="output_embeddings")
    output_embedding_PC1, variance_contribs = get_unambiguous_embedding_by_PC1_and_variance_contributions(output_embeddings_of_target_tokens)
...
if IN_COMPARISON_MODE or USE_SELF_ATTENTION_ON_OUTPUT_EMBEDDING:
    output_embeddings_of_target_tokens = get_embeddings_of_words(model, tokenizer, target_tokens, type="output_embeddings")
    output_embedding_self_attention = get_unambiguous_embedding_by_self_attention(output_embeddings_of_target_tokens)
...
if IN_COMPARISON_MODE or USE_PCA_THRESHOLD_ON_INPUT_EMBEDDING:
    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_average.to(model.device), input_embedding_PCA_threshold.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and PCA threshold ({explained_variance_threshold}): {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_PCA_threshold.to(model.device) - input_embedding_PCA_kneed.to(model.device), p=2)
    print(f"Euclidean Distance - Average and PCA threshold ({explained_variance_threshold}): {distance.item():.4f}\n")

if IN_COMPARISON_MODE or USE_PCA_KNEED_ON_INPUT_EMBEDDING:
    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_average.to(model.device), input_embedding_PCA_kneed.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and PCA kneed: {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_PCA_threshold.to(model.device) - input_embedding_PCA_kneed.to(model.device), p=2)
    print(f"Euclidean Distance - Average and PCA kneed: {distance.item():.4f}\n")

if IN_COMPARISON_MODE or USE_PC1_ON_INPUT_EMBEDDING:
    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_average.to(model.device), input_embedding_PC1.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and PC1: {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_average.to(model.device) - input_embedding_PC1.to(model.device), p=2)
    print(f"Euclidean Distance - Average and PC1: {distance.item():.4f}\n")

if IN_COMPARISON_MODE or USE_SELF_ATTENTION_ON_INPUT_EMBEDDING:
    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_average.to(model.device), input_embedding_self_attention.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and Self-attention: {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_average.to(model.device) - input_embedding_self_attention.to(model.device), p=2)
    print(f"Euclidean Distance - Average and Self-attention: {distance.item():.4f}\n")

if IN_COMPARISON_MODE:
    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_PCA_threshold.to(model.device), input_embedding_PCA_kneed.to(model.device), dim=0)
    print(f"Cosine Similarity - PCA threshold ({explained_variance_threshold}) and PCA kneed: {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_PCA_threshold.to(model.device) - input_embedding_PCA_kneed.to(model.device), p=2)
    print(f"Euclidean Distance - PCA threshold ({explained_variance_threshold}) and PCA kneed: {distance.item():.4f}\n")

    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_PCA_threshold.to(model.device), input_embedding_self_attention.to(model.device), dim=0)
    print(f"Cosine Similarity - PCA threshold ({explained_variance_threshold}) and Self-attention: {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_PCA_threshold.to(model.device) - input_embedding_self_attention.to(model.device), p=2)
    print(f"Euclidean Distance - PCA threshold ({explained_variance_threshold}) and Self-attention: {distance.item():.4f}\n")

    input_embedding_similarity = torch.nn.functional.cosine_similarity(input_embedding_PCA_kneed.to(model.device), input_embedding_self_attention.to(model.device), dim=0)
    print(f"Cosine Similarity - PCA kneed and Self-attention: {input_embedding_similarity.item():.4f}")
    distance = torch.norm(input_embedding_PCA_kneed.to(model.device) - input_embedding_self_attention.to(model.device), p=2)
    print(f"Euclidean Distance - PCA kneed and Self-attention: {distance.item():.4f}\n")

print("\nComparison of the output embeddings:")
if IN_COMPARISON_MODE or USE_PCA_THRESHOLD_ON_OUTPUT_EMBEDDING:
    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_average.to(model.device), output_embedding_PCA_threshold.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and PCA threshold ({explained_variance_threshold}): {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_PCA_threshold.to(model.device) - output_embedding_PCA_kneed.to(model.device), p=2)
    print(f"Euclidean Distance - Average and PCA threshold ({explained_variance_threshold}): {distance.item():.4f}\n")

if IN_COMPARISON_MODE or USE_PCA_KNEED_ON_OUTPUT_EMBEDDING:
    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_average.to(model.device), output_embedding_PCA_kneed.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and PCA kneed: {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_PCA_threshold.to(model.device) - output_embedding_PCA_kneed.to(model.device), p=2)
    print(f"Euclidean Distance - Average and PCA kneed: {distance.item():.4f}\n")

if IN_COMPARISON_MODE or USE_PC1_ON_OUTPUT_EMBEDDING:
    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_average.to(model.device), output_embedding_PC1.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and PC1: {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_average.to(model.device) - output_embedding_PC1.to(model.device), p=2)
    print(f"Euclidean Distance - Average and PC1: {distance.item():.4f}\n")

if IN_COMPARISON_MODE or USE_SELF_ATTENTION_ON_OUTPUT_EMBEDDING:
    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_average.to(model.device), output_embedding_self_attention.to(model.device), dim=0)
    print(f"Cosine Similarity - Average and Self-attention: {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_average.to(model.device) - output_embedding_self_attention.to(model.device), p=2)
    print(f"Euclidean Distance - Average and Self-attention: {distance.item():.4f}\n")

if IN_COMPARISON_MODE:
    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_PCA_threshold.to(model.device), output_embedding_PCA_kneed.to(model.device), dim=0)
    print(f"Cosine Similarity - PCA threshold ({explained_variance_threshold}) and PCA kneed: {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_PCA_threshold.to(model.device) - output_embedding_PCA_kneed.to(model.device), p=2)
    print(f"Euclidean Distance - PCA threshold ({explained_variance_threshold}) and PCA kneed: {distance.item():.4f}\n")

    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_PCA_threshold.to(model.device), output_embedding_self_attention.to(model.device), dim=0)
    print(f"Cosine Similarity - PCA threshold ({explained_variance_threshold}) and Self-attention: {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_PCA_threshold.to(model.device) - output_embedding_self_attention.to(model.device), p=2)
    print(f"Euclidean Distance - PCA threshold ({explained_variance_threshold}) and Self-attention: {distance.item():.4f}\n")

    output_embedding_similarity = torch.nn.functional.cosine_similarity(output_embedding_PCA_kneed.to(model.device), output_embedding_self_attention.to(model.device), dim=0)
    print(f"Cosine Similarity - PCA kneed and Self-attention: {output_embedding_similarity.item():.4f}")
    distance = torch.norm(output_embedding_PCA_kneed.to(model.device) - output_embedding_self_attention.to(model.device), p=2)
    print(f"Euclidean Distance - PCA kneed and Self-attention: {distance.item():.4f}\n")
